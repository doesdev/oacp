...
var app, Model, logger, access

// Exports
module.exports = function (thisApp, controller, whitelist) {
...
Controller.new = function () {
  ...
  Model = app.models[Class.name]
  controller.channel = app.channels[Class.name]
  console.log('hertis', app)
  controller._listen()
...
Controller.prototype._channelListeners = function () {
  ...
  self.on('error', function (req, res) {
    app.server.http.emit('error', req, res)
    return
  })
...
Controller.prototype._clean = function (params) {
  var self = this
  var paramsOut = {}
...
Controller.prototype._search = function (req, res) {
  var self = this
  if (self.beforeSearch && self.beforeSearch(req, res) === false) return
  var finder = Model.findAll(req.query)
  finder.once('error', function () { self.emit('error', req, res) })
...
Controller.prototype._create = function (req, res) {
  ...
  if (self.beforeCreate && self.beforeCreate(req, res) === false) return
  if (!access.allow(req.auth, req.params, self.createRules)) {
    return self.emit('unauthorized', req, res)
  }
  var params = self._clean(req.params)
  var record = Model.new(params)
  record.new_record = true
  record.once('error', function () { self.emit('error', req, res) })
...
Controller.prototype._read = function (req, res) {
  var self = this
  if (!self._paramsValid(req, res)) return
  if (self.beforeRead && self.beforeRead(req, res) === false) return
  var id = parseInt(req.params.id, 10)
  var record = Model.find(id)
  record.once('error', function (e) { self.emit('error', req, res, e) })
...
Controller.prototype._update = function (req, res) {
  var self = this
  if (!self._paramsValid(req, res)) return
  if (self.beforeUpdate && self.beforeUpdate(req, res) === false) return
  var id = parseInt(req.params.id, 10)
  var params = self._clean(req.params)
  var record = Model.find(id)
  record.once('error', function () { self.emit('error', req, res) })
...
Controller.prototype._delete = function (req, res) {
  var self = this
  if (!self._paramsValid(req, res)) return
  if (self.beforeDelete && self.beforeDelete(req, res) === false) return
  var id = parseInt(req.params.id, 10)
  var record = Model.find(id)
  record.once('error', function () { self.emit('error', req, res) })
  record.once('data', function () {
