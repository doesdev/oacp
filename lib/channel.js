...
Channel.new = function () {
  ...
  channel.http = {
    server: app.server.http.express,
    authenticate: app.server.http.authenticate
  }
  channel.initRoutes()
  return channel
}
...
Channel.prototype.initRoutes = function () {
  ...
  self.http.server.get(channelRoot + '_validate/', (req, res) => res.send(true))
  // SEARCH
  // GET: /channel
  self.http.server.get(channelRoot, function (req, res) {
    self.http.authenticate(req, res)
  ...
  // CREATE
  // POST: /channel
  self.http.server.post(channelRoot, bodyParser, function (req, res) {
    self.http.authenticate(req, res)
    req.once('authenticated', function (req, res) {
  ...
  // READ
  // GET: /channel/:id
  self.http.server.get(channelRoot + ':id', function (req, res) {
    self.http.authenticate(req, res)
    req.once('authenticated', function (req, res) {
  ...
  // UPDATE
  // PUT: /channel/:id
  self.http.server.put(channelRoot + ':id', bodyParser, function (req, res) {
    self.http.authenticate(req, res)
    req.once('authenticated', function (req, res) {
      Object.keys(req.body).map(function (k) { req.params[k] = req.body[k] })
  ...
  // DELETE
  // DELETE: /channel/:id
  self.http.server.delete(channelRoot + ':id', function (req, res) {
    self.http.authenticate(req, res)
    req.once('authenticated', function (req, res) {
